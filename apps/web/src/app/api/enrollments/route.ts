...
const EnrollmentsQuerySchema = z
  .object({
    // Admins may view a specific user's enrollments; everyone else always
    // sees their own (the param is ignored for non-admins).
 ...
 *
 * POST /api/enrollments — enroll in a course. Idempotent: re-enrolling
 * returns the existing enrollment. 404 when the course is not published
 * (students) or does not exist.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return env.respond(enrollments, 200, { tenantId: gate.user.tenantId, userId: gate.user.id });
  } catch (err) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!enrollment) return env.respond({ error: "Course not found or not available", requestId: env.requestId }, 404, { tenantId: gate.user.tenantId, userId: gate.user.id });

    return env.respond(enrollment, 201, { tenantId: gate.user.tenantId, userId: gate.user.id });
  } catch (err) {
    return env.capture(err);
  }
}
