...
Five shipped-artifact defects that would have broken any fresh VPS/CI run of the prod stack, caught by booting `docker-compose.prod.yml` on localhost (no VPS required for the build/compose/endpoint layer). All fixed on `feat/m6-deployment`.
