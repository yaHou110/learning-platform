...
### Fixed
- **`deploy-vercel.yml` build target restored (`--prod`).** `vercel build` without `--prod` emits a *preview*-targeted `.vercel/output`; the subsequent `vercel deploy --prebuilt --prod` then fails with "The `--prebuilt` option was used with the target environment `production`, but the prebuilt output ... was built with target environment `preview`" (first observed on the fixed pipeline's very first green-quality-gate run). The Build Project Artifacts step now runs `vercel build --yes --prod` so the prebuilt output's target matches the production deploy. Verified locally and end-to-end in CI: the smoke test (`/api/health` + `/api/ready`) passes against the new production deployment.
- **Vercel production build unblocked — `/.well-known/security.txt` is now a static file.** The App Router route handler at `apps/web/src/app/.well-known/security.txt/route.ts` (a dot-directory path) broke `vercel build` with `Error: Unable to find lambda for route: /.well-known/security.txt` — the Vercel CLI cannot map hidden-dir routes to serverless functions, so every production deployment failed with a build error. The RFC 9116 payload now ships as `apps/web/public/.well-known/security.txt` (same content, served statically, no lambda needed). Middleware already treated the path as public and still applies the security headers; the Docker standalone image already copies `apps/web/public`. Locally verified: `vercel build` green, `security.txt` present under `.vercel/output/static/.well-known/`.
- **`deploy-vercel.yml` deployment-URL resolution fixed.** The "Get Deployment URL" step ran `vercel ls --scope=<org-id> --meta=gitCommitSha=…` — `--scope` needs a team slug, not the org id, and `--meta` is not a valid `vercel ls` filter — then fell back to a hard-coded `https://learning-platform-web.vercel.app`, a stale third-party domain that 404s every `/api/health` smoke probe (the real production URL is `learning-platform-5jarnrjfx-yahou.vercel.app`-style, resolved from the project). The deploy steps now capture the deployment URL directly from `vercel deploy --prebuilt` stdout (the CLI prints it as the final line) and fail loudly if it cannot be extracted; preview and production each capture their own. Added `--yes` to every vercel CLI invocation so CI is non-interactive.
- **Legacy VPS deploy workflow (`deploy.yml`) no longer runs on push.** It requires `PROD_HOST`/`PROD_USER`/`PROD_SSH_KEY`/`GHCR_TOKEN` secrets that are not configured on this repo, so every push to main failed that job (red X on every commit). ADR-0018 made Vercel + Railway the production path; the workflow is now manual (`workflow_dispatch`) only, with a comment explaining why.
- **DB diagnostic workflow bugs fixed.** `db-diagnostic.yml` had a `PGASSWORD` typo that made the `sslmode=require` psql attempt silently useless, and tests 3–5 hard-coded a specific Railway `tokaido.proxy.rlwy.net:45305` endpoint that no longer matches the live `DATABASE_URL` secret. All probes now derive host/port from `DATABASE_URL` (with a parse guard) and the typo is fixed.
- **CI Node bump 20 → 22.** GitHub Actions deprecated Node 20 on hosted runners (the actions are already being force-run on Node 24); `deploy-vercel.yml`, `governance.yml`, `db-migrate.yml`, and `db-diagnostic.yml` now pin `node-version: '22'` to clear the deprecation annotations.
...
### Added
- **Hawza identity, dark mode, and richer dashboards (UI refresh).** Class-based dark mode across the whole app via a `ThemeToggle` (persisted in `localStorage`, system-preference default, no-FOUC init script in `layout.tsx`) with dark variants on every page (dashboard, catalog, courses, admin, login). The login page was rebuilt as a modern split layout with icon inputs, a password-visibility toggle, and a demo-account card (dev only). The dashboard was redesigned around a persistent RTL sidebar (`AppShell` — role-aware nav, user profile, theme toggle, sign-out), stat cards, per-course progress bars, and an exact Jalali date via `Intl.DateTimeFormat("fa-IR")` (a hand-rolled converter that was off by a year was removed). The course catalog gained a client-side search box, admin status-filter chips with live counts, and rich cards (status badge, lesson count, progress bar, enroll/continue/manage actions) driven by a server-action enroll flow with `revalidatePath`. Hawza identity was brought into the branding: a new `IslamicPattern` component (tiled eight-pointed khatam girih SVG, `currentColor`-adaptive), a new mosque-dome `Mosque` icon used as the logo mark across login and `AppShell`, a `DomeSkyline` silhouette, the hadith «طَلَبُ الْعِلْمِ فَرِيضَةٌ عَلَى كُلِّ مُسْلِمٍ» with Persian translation on the login panel, and a `#حوزه` community pill (desktop panel + mobile header). The enroll-card markup uses a stretched-link pattern to avoid invalid nested anchors (a hydration error).
...
- **SPRINT-002 S3 — Security hardening (security.txt contact, CSP nonces, Dependabot).** RFC 9116 `/.well-known/security.txt` now carries the real maintainer contact (`mailto:dani.rassell@gmail.com`) instead of a placeholder. The static CSP in `next.config.mjs` `headers()` was replaced by a per-request CSP generated in middleware using a 128-bit Web Crypto nonce (Edge Runtime-compatible), injected into both `script-src` and `style-src`. `unsafe-inline` retained as a fallback (CSP3 browsers ignore it when a nonce is present). `.github/dependabot.yml` added for npm + github-actions ecosystems (weekly schedule, grouped minor/patch PRs) — replaces the retired `pnpm audit`. HSTS pre-enabled (safe to ship before TLS — browsers ignore it over HTTP). Verified: live smoke against `next start` confirms nonce-bearing `Content-Security-Policy`, RFC 9116 fields on `/.well-known/security.txt`, HSTS/X-Frame/XSS/Permissions-Policy headers present, `pnpm verify` green. Evidence: `docs/06-sprints/SPRINT-002-feature-sprint/evidence/S3-security-network-hardening/`.
